...
namespace proyecto_final
{
    public partial class Form1 : Form
    {
        ...
        private void Timer1_Tick(object sender, EventArgs e)
        {
            ...
            {
                if (contador < numeroDestino && ((esBomba1 && capacidadBomba1 > 0) || (!esBomba1 && capacidadBomba2 > 0)))
                {
                    contador++;
                ...
                {
                    timer1.Stop();
                    MessageBox.Show("El contador ha llegado al número objetivo", "Completado");
        ...
        private void timerVP_Tick(object sender, EventArgs e)
        {
            ...
            {
                // Intentar convertir el texto del TextBox TxtCantidadVP a un float
                if (float.TryParse(TxtCantidadVP.Text, out float cantidadSolicitada))
                {
                    // Calcular el total de galones basado en la cantidad ingresada y el precio por galón
                    float galonesCalculados = cantidadSolicitada / precioDelDia;

                    // Verificar si hay suficiente capacidad en la bomba
                    if ((esBomba1 && capacidadBomba1 >= galonesCalculados) || (!esBomba1 && capacidadBomba2 >= galonesCalculados))
                    {
                        ...
                        // Calcular el total a pagar
                        float total = cantidadSolicitada;

                        // Mostrar el total en el TextBox TxtTotalVP
                        TxtTotalVP.Text = "Q " + total.ToString("F2");
                        ...
                        controlDistribucion.GuardarVenta(TxtNombreVP.Text, TxtApellidoVP.Text, TxtNitVP.Text, TxtTelefonoVP.Text,
                                                          TxtPrecioDelDiaVP.Text, galonesCalculados.ToString("F2"), tipo_ventas, string.Empty,
                                                          total.ToString("F2"), TxtFechaVP.Text, TxtHoraVP.Text, codigo_bomba.ToString());
                    }
                    else
                    {
                        // Si no hay suficiente capacidad en la bomba, mostrar un mensaje de error
                        MessageBox.Show("No hay suficiente capacidad en la bomba para completar la cantidad solicitada.", "Error");
                    }
                }
                else
                {
                    // Mostrar un mensaje de error si la cantidad solicitada no es válida
                    MessageBox.Show("Por favor, ingresa una cantidad válida.", "Error");
                }
            }
            else
            {
